...
control_data_path=os.path.join(parent_path, 'PARKINSON_HW', 'hw_dataset', 'control')
parkinson_data_path=os.path.join(parent_path, 'PARKINSON_HW', 'hw_dataset', 'parkinson')
...
def find_velocity(f):
    ...
    print("----------------------------------")
    print(data_pat)
    initial_timestamp=data_pat['Timestamp'].values[0]
...
def find_acceleration(f):
    ...
    #initial_timestamp=data_pat['Timestamp'].values[0]
    #data_pat['Timestamp'].values=data_pat['Timestamp'].values- initial_timestamp
    accl = []
    horz_Accl =  []
    vert_Accl = []
    magnitude = []
...
def NCA_per_halfcircle(f):
    ...
    #data_pat=data_pat[data_pat["Test_ID"]==0]    # Use only static test
    #initial_timestamp=data_pat['Timestamp'].values[0]
    #data_pat['Timestamp']=data_pat['Timestamp'].values- initial_timestamp
    accl = []
    nca = []
    temp_nca = 0
    basex = data_pat['X'].values[0]
